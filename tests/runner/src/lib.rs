...
pub fn run_test_kernel(kernel_binary_path: &str) {
    ...
    run_test_kernel_on_pxe(kernel_binary_path);
...
pub fn run_test_kernel_on_pxe(kernel_binary_path: &str) {
    let kernel_path = Path::new(kernel_binary_path);
    let tftp_folder = kernel_path.with_extension("tftp");
    bootloader::create_pxe_tftp_folder(kernel_path, &tftp_folder).unwrap();

    let mut run_cmd = Command::new("qemu-system-x86_64");
    run_cmd
        .arg("-netdev")
        .arg(format!(
            "user,id=net0,net=192.168.17.0/24,tftp={},bootfile=pxe_first_stage",
            tftp_folder.display()
        ))
        .arg("-device")
